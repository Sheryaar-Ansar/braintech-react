  ...
  return (
    <div>
      ...
      <div className={`absolute top-0 left-0 w-full h-20 flex justify-between items-center pl-5 lg:pl-7 pr-5 lg:pr-20 ${mode ? 'bg-gray-900' : 'bg-gray-200'} transition-all duration-300`}>
        <button onClick={NavigateToHome} className='mt-10'><img src={logo} className='w-[200px] h-[200px]' /></button>
        ...
        <div className='flex justify-center items-center'>
          <Toggle/>
          <h1 className='flex cursor-pointer' onClick={()=>dispatch(cartSlider())}><CiShoppingCart className='text-3xl ml-5'/><span className='bg-blue-300 rounded-full h-5 w-5 flex justify-center items-center text-sm -ml-3 font-medium'>{cartItems.length}</span> </h1>
